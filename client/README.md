...
``` comments 11-Feb-2024

1- update view for user while not logged in    ------ Done

candidtae
edit profile photo >> front end
sorting, serch , candidate, front , back

examiner
edit profile photo >> front end
<!-- badge holder >> view cand , view badge , frontend       >>>>>>>>> done -->
refactor >>> examiner/candidateBadges sorting , searching , filtering 
institute list >>> fe ela5er


provider
search ,sort ??? badge list front back
search , sort  >>> pending >> candidate name , examiner name , front back
search , sort  >>> declined>> candidate name , examiner name , front back
holder badege , view candidate, front end 

profile photo , holders badge , provider >> frontend
styling actins >>> route academy/declined-badges , frontend

<!--  i have error when render cnadidatephoto  ExaminerViewCandidateProfile  ther some data can not get  dnot know hyw ??? the data in paylod and json
 -->